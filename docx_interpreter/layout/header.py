"""
Header model for DOCX documents.

Handles header content, positioning, styling, and inheritance.
"""
...
class Header:
    ...
    def __init__(self, header_type: HeaderType = HeaderType.DEFAULT, 
        ...
        Args:
            ...
            section_number: Section number this header belongs to
        """
        ...
        self.alignment = HeaderAlignment.LEFT
        self.font = {}
        ...
        logger.debug(f"Header initialized: {header_type.value} for section {section_number}")
    ...
    def add_content(self, content) -> None:
        ...
        if isinstance(content, dict):
            content['header_index'] = len(self.content)
            content['header_type'] = self.header_type.value
        ...
        logger.debug(f"Content added to header: {content.get('type', 'unknown') if isinstance(content, dict) else 'object'}")
    
    def add_text(self, text: str, style: Optional[Dict[str, Any]] = None) -> None:
        """
        Add text content to header.
        ...
        logger.debug(f"Text added to header: {text[:50]}...")
    ...
    def set_position(self, position_or_x, y=None, width=None, height=None) -> None:
        ...
        logger.debug(f"Header position set: {self.position}")
    ...
    def set_style(self, style: Dict[str, Any]) -> None:
        ...
        logger.debug(f"Header style updated: {style}")
    
    def set_alignment(self, alignment: HeaderAlignment) -> None:
        """
        ...
        logger.debug(f"Header alignment set to {alignment.value}")
    ...
    def set_font(self, font_or_family, font_size=None, 
        ...
        logger.debug(f"Header font set: {font_family} {font_size}pt")
    ...
    def get_header_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'font': self.font.copy(),
            'parent_header': self.parent_header.header_type.value if self.parent_header else None,
            'inherited_properties': list(self.inherited_properties)
        }
    
    def set_parent_header(self, parent_header: 'Header') -> None:
        """
        Set parent header for inheritance.
        
        Args:
            parent_header: Parent header instance
        """
        self.parent_header = parent_header
        self.inherits_from_parent = True
        self._inherit_properties()
        logger.debug(f"Parent header set: {parent_header.header_type.value}")
    
    def _inherit_properties(self) -> None:
        """Inherit properties from parent header."""
        if not self.parent_header:
            return
        
        # Inherit style properties
        for prop in ['font_family', 'font_size', 'font_color', 'background_color']:
            if prop not in self.inherited_properties and prop in self.parent_header.style:
                self.style[prop] = self.parent_header.style[prop]
                self.inherited_properties.add(prop)
        
        # Inherit position properties
        for prop in ['x', 'y', 'width', 'height']:
            if prop not in self.inherited_properties and prop in self.parent_header.position:
                self.position[prop] = self.parent_header.position[prop]
                self.inherited_properties.add(prop)
    
    def clear_content(self) -> None:
        """Clear all content from header."""
        self.content.clear()
        self.content_order.clear()
        logger.debug("Header content cleared")
    ...
    def get_content_height(self) -> float:
        ...
        for content in self.content:
            if content.get('type') == 'text':
                ...
                total_height += content.get('height', 20)
            elif content.get('type') == 'table':
                rows = content.get('rows', 0)
                total_height += rows * 6  # Estimate 6mm per row
        
        return float(total_height)
    
    def has_overflow(self) -> bool:
        """Check if content overflows header area."""
        # Default header height if not specified
