...
class TestHTMLRendererNew:
    ...
    def test_render_with_paragraphs(self, mock_document):
        ...
        para1.runs = []  # Empty runs list - renderer should use get_text()
        para1.text = None
    
        para2 = Mock()
        para2.get_text = Mock(return_value="Paragraph 2")
        para2.runs = []  # Empty runs list - renderer should use get_text()
        para2.text = None
    
        mock_document.get_paragraphs = Mock(return_value=[para1, para2])
        mock_document.get_text = Mock(return_value="")  # Ensure this doesn't interfere
        # Add placeholder_values attribute to avoid Mock error in PlaceholderResolver
        ...
        # When editable=False, renderer uses _collect_paragraphs_with_formatting
        # which should extract text from get_text() when runs is empty
        assert "Paragraph 1" in html_content or "Paragraph 2" in html_content
        # The renderer should include paragraph text
        assert len(html_content) > 100  # Should have some content
